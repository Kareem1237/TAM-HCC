...
if df_file and specs_file and agendas_file:
    ...
    try:
        ...
        # Safely convert to int, handling any non-numeric values
        try:
            ...
            # Convert back to regular int for non-null values
            final_subs_sheet['practitioner_substitute_id'] = final_subs_sheet['practitioner_substitute_id'].astype(str)
        except Exception as e:
            st.warning(f"⚠️ Warning: Could not convert practitioner_substitute_id to integer: {str(e)}")
            final_subs_sheet['practitioner_substitute_id'] = final_subs_sheet['practitioner_substitute_id'].astype(str)
